    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
            if isinstance(data[0], (dict, OrderedDict)):
                ...
                if seq.dataset in ['biodrone']:
                    bbox_file = os.path.join(base_results_path, f'result/{seq.name.replace("frame_", "LGTrack_")}.txt')
        ...
        if key == 'all_scores':
            ...
            if isinstance(data[0], dict):
                ...
                if seq.dataset in ['biodrone']:
                    timings_file = os.path.join(base_results_path, f'time/{seq.name.replace("frame_", "LGTrack_")}.txt')
                else:
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k']:
                ...
                bbox_file = '{}/{}/{}.txt'.format(tracker.results_dir, seq.dataset, seq.name.replace('frame_', 'LGTrack_'))
            else:
