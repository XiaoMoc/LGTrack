...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    save_epochs = [50, 60, 70, 79, 90, 100, 110, 159, 239]
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False, resume = False):
        ...
        """
        """
        resume: True 表示断点续训（恢复 epoch 与 optimizer，strict=True）；
                False 表示微调（不恢复 epoch 与 optimizer，strict=False）
        """
        net = self.actor.net.module if multigpu.is_multi_gpu(self.actor.net) else self.actor.net
        ...
        if resume:
        # 续训：不忽略 epoch / optimizer
            strict = True
        else:
        # 微调：忽略 epoch / optimizer，并从 epoch 0 开始
            ignore_fields.extend(['epoch', 'optimizer'])
            strict = False
        ...
        if resume and ('epoch' in fields):
            ...
            print(f"Resumed from {checkpoint_path}, continue training at epoch {self.epoch}")
        else:
            self.epoch = 0
            if self.lr_scheduler is not None:
                try:
            ...
            for loader in self.loaders:
                if isinstance(loader.sampler, DistributedSampler):
                    loader.sampler.set_epoch(self.epoch)
            print(f"Loaded weights from {checkpoint_path} for finetune, starting training from epoch 0")
