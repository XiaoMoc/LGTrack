...
# network related
from lib.models.lgtrack import build_lgtrack
# forward propagation related
from lib.train.actors import LGTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "lgtrack":
        # =====distillation====
        is_distill_training = cfg.MODEL['IS_DISTILL']    # false
        if is_distill_training:
            ...
            net_teacher = build_lgtrack(cfg_teacher)
            root_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../..'))
            checkpoint = torch.load(
               # os.path.join(root_path, f'teacher_model/{teacher_backbone}/LGTrack_ep0300.pth.tar'),
                map_location="cpu"
            )
    ...
    if settings.script_name == "lgtrack":
        ...
        actor = LGTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
        if is_distill_training:
